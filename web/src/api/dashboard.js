...
import dashboardGridOptions from "../components/Redash/Dashboard/dashboard-grid-options";
...
const saveOrCreateUrl = (data) => {
  return data.slug ? `/api/dashboards/${data.slug}` : "/api/dashboards"
};
const DashboardService = {
  get: ({ slug }) =>
    axios.get(`/api/dashboards/${slug}`).then(transformResponse),
  getByToken: ({ token }) =>
    axios.get(`/api/dashboards/public/${token}`).then(transformResponse),
  save: (data) =>
    axios.post(saveOrCreateUrl(data), data).then(transformResponse),
  delete: ({ slug }) =>
    axios.delete(`/api/dashboards/${slug}`).then(transformResponse),
  query: (params) =>
...
Dashboard.prototype.getParametersDefs = function getParametersDefs(urlParams) {
  const globalParams = {};
...
Dashboard.prototype.addWidget = function addWidget(
  textOrVisualization,
  options = {}
) {
  const props = {
