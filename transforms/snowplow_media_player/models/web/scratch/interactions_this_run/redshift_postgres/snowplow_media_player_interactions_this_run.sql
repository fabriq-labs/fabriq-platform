...
, prep as (
  ...
  select
    ...
    {% elif var("snowplow__enable_whatwg_media") %}
      ...
      me.fullscreen as is_full_screen,
 ...
 {{ dbt_utils.generate_surrogate_key(['p.page_view_id', 'p.media_id' ]) }} play_id,
  ...
  coalesce(cast(case when p.is_full_screen then round(piv.weight_rate * p.duration / 100) end as {{ type_int() }}), 0) as play_time_sec_fullscreen
