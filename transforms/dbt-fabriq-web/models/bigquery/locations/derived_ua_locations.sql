{{ config(materialized='incremental',unique_key = ['app_id','ga_date','ga_country'], schema='derived') }}
...
temp_locations as (   
	SELECT *
	    ...
	    SUM(engaged_time_in_s) / COUNT(DISTINCT domain_sessionid) as  ga_avgsessionduration,
	    SUM(CASE WHEN page_views = 1 THEN 1 ELSE 0 END) / COUNT(DISTINCT domain_sessionid) AS ga_bouncerate,
	    ...
	    COUNT(domain_sessionid) / COUNT(DISTINCT domain_userid) as ga_sessionsperuser,
	...
	FULL OUTER JOIN (SELECT
	    ...
	    SUM(engaged_time_in_s) / COUNT(DISTINCT domain_userid) as ga_avgtimeonpage,
	    COUNT(DISTINCT CASE WHEN page_view_in_session_index = page_views_in_session THEN page_view_id END) / count(DISTINCT page_view_id) as ga_exitrate,
