...
with web_sessions as (
    select * from {{ ref('snowplow_web_sessions') }}
	 {% if is_incremental() %}
    where  date(start_tstamp) >= (select max(date(ga_date)) from {{this}})
    {% endif %}
),
...
web_page_views as (
	...
	{% if is_incremental() %}
    where  date(collector_tstamp) >= (select max(date(ga_date)) from {{this}})
    {% endif %}
),
...
temp_traffic_sources as (   
	SELECT *
	    ...
	    Avg(page_views::double precision) as ga_pageviewspersession,
