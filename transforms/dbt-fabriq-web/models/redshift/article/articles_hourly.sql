...
referrers AS (
    SELECT
        ...
        DATE_PART('hour', customer_tstamp) as hour,
        content_id,
        COALESCE(refr_urlhost, 'Direct') AS referrer,
        COUNT(DISTINCT domain_userid) AS cnt
    FROM content
    WHERE  date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp), referrer, content_id
),
countries AS (
    SELECT
        TO_CHAR(customer_tstamp, 'YYYY-MM-DD') AS period_date,
    ...
    FROM content
    WHERE date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp), country, content_id
),
city AS (
    SELECT
        TO_CHAR(customer_tstamp, 'YYYY-MM-DD') AS period_date,
        ...
        content_id
    FROM content
    GROUP BY app_id,content_id, geo_country, geo_city, TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp)
),
ranked_cities AS (
    SELECT
        *,
...
session_counts AS (
    ...
    FROM content
    WHERE  date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp), domain_sessionid, content_id
...
article_hourly AS (
   SELECT
    ...
    FROM content cba
    WHERE date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY app_id, TO_CHAR(customer_tstamp, 'YYYY-MM-DD'), DATE_PART('hour', customer_tstamp), cba.content_id
)
...
SELECT
    ...
    (SELECT '{' || LISTAGG('"' || referrer || '": ' || cnt, ', ') WITHIN GROUP (ORDER BY referrer) || '}' FROM referrers WHERE referrers.period_date = article_hourly.period_date and referrers.content_id = article_hourly.article_id and referrers.hour = article_hourly.hour ) AS referrer_distribution,
    (SELECT '{' || LISTAGG('"' || country || '": ' || cnt, ', ') WITHIN GROUP (ORDER BY country) || '}' FROM countries WHERE countries.period_date = article_hourly.period_date and countries.content_id = article_hourly.article_id and countries.hour = article_hourly.hour ) AS country_distribution,
    (
	select
		'{' || LISTAGG('"' || country || '": ' || cities, ', ') WITHIN GROUP (ORDER BY country) || '}'
	from country_wise_city c where
		c.period_date = article_hourly.period_date
		and c.content_id = article_hourly.article_id and c.hour = article_hourly.hour) as city_distribution,
    s.org_id
FROM article_hourly
