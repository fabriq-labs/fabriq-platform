...
total_time_spent as (
	select
		...
		DATE_PART('hour', customer_tstamp) as period_hour,
	...
	group by
		...
		DATE_PART('hour', customer_tstamp),
		author
),
resf_source as (
SELECT 
...
FROM content c
GROUP BY app_id, author, TO_CHAR(customer_tstamp,'YYYY-MM-DD'), referrer, DATE_PART('hour', customer_tstamp)
),
...
group by
	...
	'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp),
	country,
	author
),
city AS (
    SELECT
        ...
        app_id
    FROM content
    GROUP BY app_id, author, geo_country, geo_city, TO_CHAR(customer_tstamp, 'YYYY-MM-DD'), DATE_PART('hour', customer_tstamp)
),
ranked_cities AS (
    SELECT
        *,
...
from content c
group by app_id, author, TO_CHAR(customer_tstamp,'YYYY-MM-DD'), DATE_PART('hour', customer_tstamp)
)
