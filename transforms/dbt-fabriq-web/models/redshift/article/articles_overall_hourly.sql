...
with content as (
    ...
    GROUP BY TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp), referrer, app_id
),
countries AS (
    SELECT
        TO_CHAR(customer_tstamp, 'YYYY-MM-DD') AS period_date,
    ...
    FROM content
    WHERE date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp), country, app_id
),
city AS (
    SELECT
        TO_CHAR(customer_tstamp, 'YYYY-MM-DD') AS period_date,
        ...
        app_id
    FROM content
    GROUP BY app_id, geo_country, geo_city, TO_CHAR(customer_tstamp, 'YYYY-MM-DD'),DATE_PART('hour', customer_tstamp)
),
ranked_cities AS (
    SELECT
        *,
...
session_counts AS (
    ...
    FROM content
    WHERE  date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY TO_CHAR(customer_tstamp,'YYYY-MM-DD'),  DATE_PART('hour', customer_tstamp),domain_sessionid, app_id
),
article_hourly AS (
    SELECT
        app_id AS site_id,
    ...
    FROM content cba
    WHERE date(customer_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY app_id, period_date, DATE_PART('hour', customer_tstamp)
)
