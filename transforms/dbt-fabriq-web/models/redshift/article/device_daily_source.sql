...
devices AS (
    ...
    FROM content
    GROUP BY TO_CHAR(custom_tstamp, 'YYYY-MM-DD'), device, content_id
),
operating_systems AS (
    SELECT
        ...
        COUNT(DISTINCT domain_userid) AS cnt
    FROM content
    GROUP BY TO_CHAR(custom_tstamp, 'YYYY-MM-DD'), operating_system, content_id
),
device_daily_source AS (
    ...
    FROM content c
    GROUP BY app_id, TO_CHAR(c.custom_tstamp, 'YYYY-MM-DD'), c.content_id
)
SELECT
device_daily_source.*,
s.org_id
