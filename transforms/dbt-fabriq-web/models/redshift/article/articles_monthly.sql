...
with content as (
    ...
    FROM content
    GROUP BY  EXTRACT(YEAR FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp), device, content_id
),
referrers AS (
    SELECT
        ...
        COUNT(DISTINCT domain_userid) AS cnt
    FROM content
    GROUP BY EXTRACT(YEAR FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp), referrer, content_id
),
countries AS (
    SELECT
         EXTRACT(MONTH FROM customer_tstamp) AS period_month,
        ...
        COUNT(DISTINCT domain_userid) AS cnt
    FROM content
    GROUP BY EXTRACT(YEAR FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp), country, content_id
),
city AS (
    SELECT
        EXTRACT(MONTH FROM customer_tstamp) AS period_month,
		...
		content_id
    FROM content
    GROUP BY app_id, content_id, geo_country, geo_city, EXTRACT(YEAR FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp)
),
ranked_cities AS (
    SELECT
        *,
...
socials AS (
    SELECT
       ...
       COALESCE(refr_source , 'Unknown') AS social,
        COUNT(DISTINCT domain_userid) AS cnt
    FROM content
    GROUP BY  EXTRACT(YEAR FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp), social, content_id
),
session_counts AS (
    SELECT
        EXTRACT(MONTH FROM customer_tstamp) AS period_month,
    ...
    FROM content
    GROUP BY EXTRACT(YEAR FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp), domain_sessionid, content_id
),
article_monthly AS (
    SELECT
        app_id AS site_id,        
    ...
    FROM content cba
    GROUP BY app_id, EXTRACT(MONTH FROM customer_tstamp), EXTRACT(YEAR FROM customer_tstamp), cba.content_id
)
