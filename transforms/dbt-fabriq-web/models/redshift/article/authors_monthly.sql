...
FROM content c
GROUP BY app_id, author, EXTRACT(MONTH FROM custom_tstamp),EXTRACT(year FROM custom_tstamp), referrer
),
...
city AS (
    SELECT
        ...
        app_id
    FROM content
    GROUP BY app_id, author, geo_country, geo_city, EXTRACT(MONTH FROM custom_tstamp),
	EXTRACT(year FROM custom_tstamp)
),
ranked_cities AS (
    SELECT
...
from content c
group by app_id, author, EXTRACT(year FROM custom_tstamp),EXTRACT(MONTH FROM custom_tstamp)
)
