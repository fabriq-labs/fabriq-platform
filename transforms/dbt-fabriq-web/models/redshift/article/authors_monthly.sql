...
FROM content c
GROUP BY app_id, author, EXTRACT(MONTH FROM customer_tstamp),EXTRACT(year FROM customer_tstamp), referrer
),
...
city AS (
    SELECT
        ...
        app_id
    FROM content
    GROUP BY app_id, author, geo_country, geo_city, EXTRACT(MONTH FROM customer_tstamp),
	EXTRACT(year FROM customer_tstamp)
),
ranked_cities AS (
    SELECT
...
from content c
group by app_id, author, EXTRACT(year FROM customer_tstamp),EXTRACT(MONTH FROM customer_tstamp)
)
...
(select '{' || LISTAGG('"' || referrer || '": ' || cnt, ', ') WITHIN GROUP (ORDER BY referrer) || '}' from source_distribution refr where refr.period_year = a.period_year and refr.period_month=a.period_month and refr.author = a.author) as source_distribution,
(select '{' || LISTAGG('"' || referrer || '": ' || cnt, ', ') WITHIN GROUP (ORDER BY referrer) || '}' from medium_distribution refr where refr.period_year = a.period_year and refr.period_month=a.period_month and refr.author = a.author) as medium_distribution,
(select '{' || LISTAGG('"' || country || '": ' || cnt, ', ') WITHIN GROUP (ORDER BY country) || '}' from countries c where c.period_year = a.period_year and c.period_month = a.period_month and c.author = a.author) as country_distribution,
