{{ config(materialized='incremental',unique_key = ['app_id', 'domain_userid', 'source', 'operating_system_class', 'page_urlhost', 'geo_country', 'geo_city', 'date'], schema='derived') }}
...
sources as (
select app_id, CURRENT_TIMESTAMP AS updated_time,date(collector_tstamp) as date, domain_userid, count(page_view_id) as page_view_count,
...
from content dc 
group by domain_userid, mkt_source, operating_system_class, page_urlhost, geo_country, geo_city, date, app_id)
