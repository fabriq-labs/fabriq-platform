...
cities as (
    select site_id, media_id, period_quarter,period_year, city, average_users
    from (
        select
            site_id, media_id, period_quarter,
            period_year,
            city,
            average_users,
            row_number() over(partition by period_quarter, period_year order by average_users desc) as rank_within_period
        from (
            SELECT
                ...
                AVG(COUNT(DISTINCT domain_userid)) OVER (PARTITION BY city, period_quarter, period_year) AS average_users
            FROM video_content
            GROUP BY 1, 2, 3, 4, 5
        ) as sub
    ) as ranked_sub
    where rank_within_period <= 5
    order by period_quarter,period_year, average_users desc
),
top_refrer_source as (
    select site_id, media_id, period_quarter, period_year,referrer, users
    from (
        select 
            site_id, media_id,
            period_quarter,
            ...
            referrer,
            users,
            row_number() over(partition by period_quarter, period_year order by users desc) as rank_within_period
        from (
            select 
                app_id as site_id, media_id,
               EXTRACT(QUARTER FROM collector_tstamp) AS  period_quarter,
            ...
            group by site_id, media_id, period_quarter,period_year, referrer
        ) as sub
    ) as ranked_sub
    where rank_within_period <= 5
    order by period_quarter,period_year, users desc
),
viewer_segments as (
    select
        domain_userid,
...
viewer_counts as (
    ...
    from
        segmented_viewers
    group by
        site_id, media_id, period_quarter,period_year, viewer_segment
),
viewer_percent as (
    select
        site_id,
...
select 
    ...
    from
      cities
    where cities.period_quarter =  v.period_quarter and cities.period_year =  v.period_year and cities.site_id = v.site_id and cities.media_id = v.media_id
    ) as top_cities,
    (
    select
