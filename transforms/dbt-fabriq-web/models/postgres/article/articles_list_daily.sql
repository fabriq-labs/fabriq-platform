...
SELECT
  ...
  (
    SELECT JSON_OBJECT_AGG(coalesce('unknown',city), average_users ORDER BY average_users DESC)
    FROM cities
    WHERE cities.period_date = article_list_daily.period_date
  ) AS top_cities,
  (
    SELECT JSON_OBJECT_AGG(coalesce('unknown',referrer), users ORDER BY users DESC)
    FROM top_refrer_source re
