{{ config(materialized='incremental',unique_key = ['site_id', 'author', 'period_year', 'refr_medium' ], sort=['site_id', 'author', 'period_year' ],
...
referer AS (
    SELECT
        ...
        page_urlpath,
        refr_urlhost,
        ROW_NUMBER() OVER (PARTITION BY refr_medium,period_year,author,site_id ORDER BY SUM(cnt) DESC) AS rn
    FROM (
        SELECT
            author,
    ...
    GROUP BY author,  period_year, refr_medium, refr_source, site_id, page_urlpath, refr_urlhost
)
SELECT
    referer.*,
    s.org_id,
