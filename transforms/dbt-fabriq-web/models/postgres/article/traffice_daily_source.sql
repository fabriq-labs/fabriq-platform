...
traffic_daily_source AS (
    SELECT
        ...
        CAST(SUM(case when page_views_in_session = 1 then 1 else 0 end) AS decimal) / CAST(COUNT(distinct c.domain_sessionid) AS decimal) as bounce_rate,
        ...
        (SELECT json_object_agg(medium, cnt)
        FROM medium_daily
        WHERE medium_daily.content_id = c.content_id) AS medium_distribution,
        (SELECT json_object_agg(social, cnt)
        FROM social_daily
        WHERE social_daily.content_id = c.content_id) AS source_distribution
    FROM content c
