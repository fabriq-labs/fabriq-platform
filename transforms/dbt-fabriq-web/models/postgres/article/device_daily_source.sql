...
device_daily_source AS (
    SELECT
        ...
        FROM devices
        WHERE devices.content_id = c.content_id) AS device_category,
        (SELECT json_object_agg(operating_system, cnt)
        FROM operating_systems
        WHERE operating_systems.content_id = c.content_id) AS operating_system
