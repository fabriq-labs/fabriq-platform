{{ config(materialized='incremental',unique_key = 'event_id', schema='derived') }}

with snowplow_web_page_views as (
    select * from {{ ref('snowplow_web_page_views') }}
	{% if is_incremental() %}
    where  date(collector_tstamp) >= COALESCE((select max(date(collector_tstamp)) from {{this}}), '{{ var('snowplow_web')['snowplow__start_date'] }}' )
    {% endif %}
),
