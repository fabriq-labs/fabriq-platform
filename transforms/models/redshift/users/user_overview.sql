...
SELECT app_id as site_id,
       ...
       CURRENT_TIMESTAMP AS updated_time,
        (
        select
            '{' || LISTAGG('"' || author || '": ' || author_views, ', ') WITHIN GROUP (ORDER BY author_views desc) || '}'
        from
            ranked_authors
        where
            ranked_authors.domain_userid = web_users.domain_userid and rn <= 3) as top_authors,
        (
        select
            '{' || LISTAGG('"' || content_category || '": ' || category_views, ', ') WITHIN GROUP (ORDER BY category_views desc) || '}'
        from
            ranked_categories
        where
            ranked_categories.domain_userid = web_users.domain_userid and rn <= 3) as top_categories                                                              
FROM   web_users
