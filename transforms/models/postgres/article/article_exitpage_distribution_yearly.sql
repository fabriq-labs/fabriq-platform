{{ config(materialized='incremental',unique_key = ['site_id', 'article_id','period_year', 'page_title', 'next_page_title', 'recirculation_count'  ], schema='public') }}
...
recirculation_counts AS (
  ...
  GROUP BY site_id, article_id, period_year, page_title, next_page_title,next_page_article_id
)
SELECT
  rc.site_id,
  rc.article_id,
