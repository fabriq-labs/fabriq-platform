{{ config(materialized='incremental',unique_key = ['site_id' ,'author','period_quarter', 'period_year'  ], schema='public') }}
...
(select JSON_OBJECT_AGG(referrer,cnt) from source_distribution refr where refr.period_year = a.period_year and refr.period_quarter=a.period_quarter and refr.author = a.author) as source_distribution,
(select JSON_OBJECT_AGG(referrer,cnt) from medium_distribution refr where refr.period_year = a.period_year and refr.period_quarter=a.period_quarter and refr.author = a.author) as medium_distribution,
(select JSON_OBJECT_AGG(COALESCE(country, 'Unknown'), cnt) from countries c where c.period_year = a.period_year and c.period_quarter = a.period_quarter and c.author = a.author) as country_distribution
from authors a
