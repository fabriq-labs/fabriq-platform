...
SELECT content_id as article_id, CAST(period_year AS integer) as period_year, refr_medium, refr_source, sum(cnt) as users, site_id, page_urlpath, refr_urlhost
FROM (
  SELECT
    content_id,
...
group by content_id, period_year, refr_medium, refr_source,site_id, page_urlpath, refr_urlhost
)

select referer.*, s.org_id, CURRENT_TIMESTAMP AS created_at from referer INNER JOIN sites s ON s.site_id = referer.site_id