...
article_hourly AS (
    ...
    FROM content cba
    JOIN session_counts ON TO_CHAR(derived_tstamp, 'YYYY-MM-DD') = session_counts.period_date and session_counts.content_id = cba.content_id
    WHERE date(derived_tstamp) >= CURRENT_DATE - INTERVAL '8 days'
    GROUP BY app_id, period_date, DATE_PART('hour', derived_tstamp), cba.content_id
)
...
SELECT
    ...
    (SELECT JSON_OBJECT_AGG(country, cnt) FROM countries WHERE countries.period_date = article_hourly.period_date and countries.content_id = article_hourly.article_id and countries.hour = article_hourly.hour ) AS country_distribution,
    s.org_id
FROM article_hourly
