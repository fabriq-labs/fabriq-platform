{{ config(materialized='incremental',unique_key = ['site_id', 'author','period_year' ], schema='public') }}
...
(select JSON_OBJECT_AGG(COALESCE(country, 'Unknown'), cnt) from countries c where c.period_year = a.period_year and c.author = a.author) as country_distribution
from authors a
