{{ config(materialized='incremental',unique_key = ['site_id', 'article_id','period_month','period_year', 'page_title', 'next_page_title', 'recirculation_count'  ], schema='public') }}
...
recirculation_counts AS (
  ...
  GROUP BY site_id, article_id, period_month, page_title, next_page_title, period_year,next_page_article_id
)
SELECT
  rc.site_id,
  rc.article_id,
  rc.next_page_article_id,
  CAST(rc.period_month AS integer) as period_month,
