...
with content as (
    ...
    GROUP BY FORMAT_TIMESTAMP('%Y-%m-%d', derived_tstamp),EXTRACT(HOUR FROM derived_tstamp) , referrer
),
countries AS (
    SELECT
        FORMAT_TIMESTAMP('%Y-%m-%d', derived_tstamp) AS period_date,
    ...
    FROM content
    WHERE DATE(derived_tstamp) >= DATE(CURRENT_TIMESTAMP()) - INTERVAL 8 DAY
    GROUP BY FORMAT_TIMESTAMP('%Y-%m-%d', derived_tstamp),EXTRACT(HOUR FROM derived_tstamp) , country
),
session_counts AS (
    SELECT
        FORMAT_TIMESTAMP('%Y-%m-%d', derived_tstamp) AS period_date,
    ...
    FROM content
    WHERE  DATE(derived_tstamp) >= DATE(CURRENT_TIMESTAMP()) - INTERVAL 8 DAY
    GROUP BY FORMAT_TIMESTAMP('%Y-%m-%d', derived_tstamp), domain_sessionid
),
article_hourly AS (
    SELECT
        app_id AS site_id,
    ...
    FROM content cba
    JOIN session_counts ON FORMAT_TIMESTAMP('%Y-%m-%d', derived_tstamp) = session_counts.period_date
    WHERE DATE(derived_tstamp) >= DATE(CURRENT_TIMESTAMP()) - INTERVAL 8 DAY
    GROUP BY app_id, period_date, EXTRACT(HOUR FROM derived_tstamp) 
)
...
SELECT
   ...
   (SELECT CONCAT(
        ...
        '}'
      ) FROM referrers WHERE referrers.period_date = article_hourly.period_date and referrers.hour = article_hourly.hour ) AS referrer_distribution,
    (SELECT CONCAT(
        '{',
        STRING_AGG(
        ...
        '}'
      ) FROM countries WHERE countries.period_date = article_hourly.period_date and countries.hour = article_hourly.hour ) AS country_distribution,
    s.org_id
FROM article_hourly
